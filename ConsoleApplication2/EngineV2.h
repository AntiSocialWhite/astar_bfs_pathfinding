			...
			while (m_bAtomActive)
			{
				...
				for (DWORD i = 0; i < events; i++)
				{
					...
					case MOUSE_EVENT:
					{
						...
						case MOUSE_MOVED:
						{
							m_poz_x = inBuf[i].Event.MouseEvent.dwMousePosition.X;
							m_poz_y = inBuf[i].Event.MouseEvent.dwMousePosition.Y;
						}
						break;
...
public:
	...
	int GetMouseX() { return m_poz_x; }
	int GetMouseY() { return m_poz_y; }
